...
export const SearchResult = ({resultItem, setMsg, setLoading}) => {
    ...
    return (
        <div align='center'>
            ...
            <Typography>Default Amount: {resultItem.serving_size_g}</Typography>
            <Typography>Food Type: {calcFoodType()}</Typography>
            <hr/>
            <SaveSearchFoodItem setLoading={setLoading} amountType={'Grams'} foodType={calcFoodType()}  setMsg={setMsg} resultItem={resultItem}/>
        </div>
    )
}

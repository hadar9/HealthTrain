...
import {
  fetchAllFoodItems,
  updateMeal,
} from "../../../../redux/reducers/FoodDiaryReducer";
...
export const MealTable = ({ meal, mealIndex }) => {
  ...
  const compareDBFoodItemWithNew = (dbItem, newItem) => {
    ...
    if (
      dbItem.amountType !== newItem.amountType ||
      dbItem.foodCalories !== newItem.foodCalories ||
      dbItem.defaultAmount !== newItem.defaultAmount
    )
      return true;
    return false;
  };
  ...
  const onSubmit = async (e) => {
    try {
      e.preventDefault();
      const isFoodItemChanged = compareDBFoodItemWithNew(
        findFoodItemByID(currentFoodItem._id),
        currentFoodItem
      );
      console.log(isFoodItemChanged);
      if (isFoodItemChanged) await createNewFoodItem();
      const { _id, defaultAmount, name, amountType, foodCalories, foodType } =
        currentFoodItem;
      const newFoodItem = {
        _id,
        defaultAmount,
      ...
      let updatedMeal = {
        ...
        foodItems: [...meal.foodItems, newMealItem],
      };
  ...
  const table = <div style={{ width: "100%" }}>{formBody}</div>;

  const addFoodButton = addFoodCell ? null : (
    <IconButton variant="outlined" onClick={() => setAddFoodCell("true")}>
